...
/**
  ...
  */
class AppkitProvingInterpreter(val secretKeys: JList[ExtendedSecretKey], val dLogInputs: JList[DLogProverInput], val dhtInputs: JList[DiffieHellmanTupleProverInput], params: ErgoLikeParameters)
    extends ErgoLikeInterpreter()(new CompiletimeIRContext)
  ...
  val secrets: Seq[SigmaProtocolPrivateInput[_ <: SigmaProtocol[_], _ <: SigmaProtocolCommonInput[_]]] = {
    val dlogs: IndexedSeq[DLogProverInput] = try JListToIndexedSeq(identityIso[ExtendedSecretKey]).to(secretKeys).map(_.key)
    catch { case _: Exception => IndexedSeq() }
    val dlogsAdditional: IndexedSeq[DLogProverInput] = try JListToIndexedSeq(identityIso[DLogProverInput]).to(dLogInputs)
    catch { case _: Exception => IndexedSeq() }
    val dhts: IndexedSeq[DiffieHellmanTupleProverInput] = try JListToIndexedSeq(identityIso[DiffieHellmanTupleProverInput]).to(dhtInputs)
    catch { case _: Exception => IndexedSeq() }
  ...
  /**
    ...
    */
  def sign(unsignedTx: UnsignedErgoLikeTransaction, boxesToSpend: IndexedSeq[ErgoBox], dataBoxes: IndexedSeq[ErgoBox], stateContext: ErgoLikeStateContext): Try[ErgoLikeTransaction] = Try {
